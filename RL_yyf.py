...
def Simulate(rl, orderbooks_df, transaction_df, quantile_df, time_frame, start_time, FeatureNum, numTrial=50,
             learn=True):
...
if __name__ == '__main__':
    ...
    for epoch in [10, 20, 50, 100, 200, 400]:
        for starting_point in [1000]:
            ...
            mdp = MDP.TradeMDP(totalVol=100, voLevel=5, timeLevel=timeLevel, priceFlex=4, timeGap=timeGap,
                               FeatureLevel=3,
            ...
            for i in range(division):
                ...
                order_book_df = orderbooks.drop(columns=['trade_price', 'trade_size', 'trade_direction']).loc[
                                starting_point + i * total_time:starting_point + (i + 1) * total_time]
                transaction_df = orderbooks[['trade_price', 'trade_size', 'trade_direction']].loc[
                                 starting_point + i * total_time: starting_point + (i + 1) * total_time]
            ...
            test_order_book_df = orderbooks.drop(columns=['trade_price', 'trade_size', 'trade_direction']).loc[
                                 test_start_time:test_end_time]
            test_transaction_df = orderbooks[['trade_price', 'trade_size', 'trade_direction']].loc[
                                  test_start_time:test_end_time]
